...
static int clock_control_ft_on(const struct device *dev,
				 clock_control_subsys_t sys)
{
	const struct clock_control_ft_config *config = dev->config;
	uint16_t id = *(uint16_t *)sys;

	sys_set_bit(config->base + FOCALTECH_CLOCK_ID_OFFSET(id),
		    FOCALTECH_CLOCK_ID_BIT(id));

	return 0;
}

static int clock_control_ft_off(const struct device *dev,
				  clock_control_subsys_t sys)
{
	const struct clock_control_ft_config *config = dev->config;
	uint16_t id = *(uint16_t *)sys;

	sys_clear_bit(config->base + FOCALTECH_CLOCK_ID_OFFSET(id),
		      FOCALTECH_CLOCK_ID_BIT(id));
...
clock_control_ft_get_status(const struct device *dev,
			      clock_control_subsys_t sys)
{
	const struct clock_control_ft_config *config = dev->config;
	uint16_t id = *(uint16_t *)sys;

	if (sys_test_bit(config->base + FOCALTECH_CLOCK_ID_OFFSET(id),
			 FOCALTECH_CLOCK_ID_BIT(id)) != 0) {
