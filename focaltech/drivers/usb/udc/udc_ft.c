...
static int udc_ft_pm_action(const struct device* dev,
...
void udc_ft_dma_write_packet(const struct device *dev, uint8_t ep_idx, uint8_t *buffer, uint16_t len)
{
    ...
    USBx->USB_DMAReg[ep_idx].DMA_CNTL = DEV_CNTL_DMAEN | DEV_CNTL_EP(ep_idx) | 
        DEV_CNTL_DIRECTION_READ | DEV_CNTL_INTERE | DEV_CNTL_BURSTMODE(0);
...
void udc_ft_dma_read_packet(const struct device *dev, uint8_t ep_idx, uint8_t *buffer, uint16_t len)
{
    ...
    USBx->USB_DMAReg[ep_idx].DMA_COUNT = len;
    USBx->USB_DMAReg[ep_idx].DMA_CNTL = DEV_CNTL_DMAEN | DEV_CNTL_EP(ep_idx) | 
                            DEV_CNTL_INTERE | DEV_CNTL_BURSTMODE(0);
...
static  int ft_udc_handle_ep0_setup(const struct device *dev)      //a in packet is sent to host
{
	...
	//LOG_WRN("setup:%02x %02x : %02x %02x %02x %02x %02x %02x",data_ptr[0], data_ptr[1], data_ptr[2], data_ptr[3],data_ptr[4],data_ptr[5],data_ptr[6],data_ptr[7]);
    ...
    if (udc_ctrl_stage_is_data_out(dev)) {     //data out
...
static int ft_udc_msg_handle_ep0(const struct device *dev)
{
    ...
    if (csr_l & DEV_CSR0_SETUPEND) {     //control transfer data process is done
...
static int ft_udc_msg_handle_fake_status_in(const struct device *dev,  struct udc_ft_msg *msg) 
{
	...
	buf = udc_buf_get(ep_cfg);
	if (unlikely(buf == NULL)) {
		LOG_ERR("ep 0x%02x queue is empty", USB_CONTROL_EP_IN);
		return 0;
	}
...
static int udc_ft_ep_enqueue(const struct device *dev,
				   ...
				   struct net_buf *buf)
{
	...
	LOG_DBG("%p enqueue [%p:%p] halted:%u, ep:%x, len:%x", dev, buf,ep_cfg,ep_cfg->stat.halted, ep_cfg->addr, buf->len);

	/*If the net_buf addresses do not match, it indicates that the previous interrupt transfer data failed to be transmitted. 
	  Perform a reset on the interrupt endpoint's net_buf.*/
	
	if(is_interrupt_ep){
	    if(udc_buf_peek(ep_cfg)!=buf){
		LOG_ERR("endpoint buf check err!!!,buf->len=%d,ep=%x,halt=%d\n",buf->len,ep_cfg->addr,ep_cfg->stat.halted);
 ...
 */
static int udc_ft_ep_dequeue(const struct device *dev,
				   struct udc_ep_config *const ep_cfg)
{
	const struct udc_ft_config *config = dev->config;
	struct net_buf *buf;
	...
	udc_ep_set_busy(ep_cfg, false); 
	LOG_ERR("dequeue ep 0x%02x", ep_cfg->addr);
 ...
 */
static int udc_ft_ep_enable(const struct device *dev,
				  struct udc_ep_config *const ep_cfg)
{
	const struct udc_ft_config *config = dev->config;
	int status;
 ...
 */
static int udc_ft_ep_disable(const struct device *dev,
				   struct udc_ep_config *const ep_cfg)
{
	const struct udc_ft_config *config = dev->config;
	int status = 0;
...
static int udc_ft_ep_set_halt(const struct device *dev,
				    struct udc_ep_config *const ep_cfg)
{
	const struct udc_ft_config *config = dev->config;
	...
	LOG_INF("Set halt ep 0x%02x", ep_cfg->addr);
	udc_ft_hal_lock(dev);
	//HW set ep stall
 ...
 */
static int udc_ft_ep_clear_halt(const struct device *dev,
				      struct udc_ep_config *const ep_cfg)
{
	const struct udc_ft_config *config = dev->config;
...
static int udc_ft_driver_preinit(const struct device *dev)
{
	...
	if (config->speed_idx == 2) {    //high_speed
		data->caps.hs = true;
		mps = 512;
	}
	else {                           //full speed
...
static void udc_ft_irq_config_func_##n(const struct device *dev)                   \
	{                                                                                          \
		IRQ_CONNECT(DT_INST_IRQN(n), DT_INST_IRQ(n, priority), ft_udbd_isr,     \
	...
	{									\
		...
		k_thread_create(&priv->thread_data,				\
				udc_ft_stack_##n,				\
				K_THREAD_STACK_SIZEOF(udc_ft_stack_##n),	\
				udc_ft_thread_##n,			\
				(void *)dev, NULL, NULL,			\
				K_PRIO_COOP(CONFIG_UDC_FT_THREAD_PRIORITY),\
				K_ESSENTIAL,					\
				K_NO_WAIT);					\
	...
	static struct udc_ep_config						\
		ep_cfg_in_##n[DT_INST_PROP(n, num_bidir_endpoints)];		\
										\
	static const struct udc_ft_config udc_ft_dev_config_##n = {	\
		.base = (FT_USBD_Type*)DT_INST_REG_ADDR(n),                             \
		...
		.irq_config_func = udc_ft_irq_config_func_##n \
										...
										\
	K_MSGQ_DEFINE(udc_ft_msgq_##n, sizeof(struct udc_ft_msg),   \
		      CONFIG_UDC_FT_MAX_QMESSAGES, 4);                 \
										\
	...
	DEVICE_DT_INST_DEFINE(n, udc_ft_driver_preinit, PM_DEVICE_DT_INST_GET(n),	\
			      &udc_data_##n, &udc_ft_dev_config_##n,		\
