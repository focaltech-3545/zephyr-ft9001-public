...
void UART_Init(UART_TypeDef *UARTx, UART_InitTypeDef *UART_InitStruct, uint32_t sys_freq)
{
    ...
    if ((UART_InitStruct->UART_FrameLength == UART_DATA_FRAME_LEN_11BIT) && (UART_InitStruct->UART_StopBits == 2))
    {
        UARTx->SCIDRH |= (1u << 6);
    }
...
int UART_IrqIsPending(UART_TypeDef *UARTx)
{
    ...
    if ((!(UARTx->SCIFSR & SCIFSR_TFULL_MASK) && (UARTx->SCIFCR2 & SCIFCR2_TXFIE)) // tx_ready
        || (((UARTx->SCIFSR & SCIFSR_RFTS_MASK) || !(UARTx->SCIFSR & SCIFSR_REMPTY_MASK)) &&
            (UARTx->SCIFCR2 & SCIFCR2_RXFIE))) // rx_ready
    {
        ret = 0;
    }
    return ret;
...
int UART_IrqTxPending(UART_TypeDef *UARTx)
{
    int ret = -1;
    if ((UARTx->SCIFSR & SCIFSR_TEMPTY_MASK) && (UARTx->SCIFSR & SCIFSR_FTC_MASK) &&
        (UARTx->SCIFSR & SCIFSR_REMPTY_MASK))
        ret = 0;
    return ret;
}
...
int UART_IrqErrCheck(UART_TypeDef *UARTx)
{
    ...
    z_err = ((UARTx->SCIFSR2 & SCISR2_FOR_MASK) ? (1 << 0) : 0) | ((UARTx->SCIFSR2 & SCISR2_FPF_MASK) ? (1 << 1) : 0) |
            ((UARTx->SCIFSR2 & SCISR2_FNF_MASK) ? (1 << 5) : 0) | ((UARTx->SCIFSR2 & SCISR2_FFE_MASK) ? (1 << 2) : 0);
