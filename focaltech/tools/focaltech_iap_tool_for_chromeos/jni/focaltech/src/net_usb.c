...
int mbedtls_usb_init(const uint32_t pid, const uint32_t vid)
{
    ...
    ret = libusb_hotplug_register_callback(NULL, LIBUSB_HOTPLUG_EVENT_DEVICE_ARRIVED | LIBUSB_HOTPLUG_EVENT_DEVICE_LEFT,
                                           0, vid, pid, LIBUSB_HOTPLUG_MATCH_ANY, usb_event_callback, NULL, &hp[0]);
...
int mbedtls_usb_exist(uint16_t *pid, uint16_t *vid)
{
    ...
    ret = libusb_get_device_list(NULL, &usb_ctx->devs);
    if (ret < 0)
    {
