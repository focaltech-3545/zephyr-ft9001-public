...
static int reset_ft_status(const struct device *dev, uint32_t id, uint8_t *status)
{
    const struct reset_ft_config *config = dev->config;

    *status = !!sys_test_bit(config->base + FOCALTECH_RESET_ID_OFFSET(id), FOCALTECH_RESET_ID_BIT(id));
...
static int reset_ft_line_assert(const struct device *dev, uint32_t id)
{
    ...
    sys_set_bit(config->base + FOCALTECH_RESET_ID_OFFSET(id), FOCALTECH_RESET_ID_BIT(id));
...
static int reset_ft_line_deassert(const struct device *dev, uint32_t id)
{
    ...
    sys_clear_bit(config->base + FOCALTECH_RESET_ID_OFFSET(id), FOCALTECH_RESET_ID_BIT(id));
...
DEVICE_DT_INST_DEFINE(0, NULL, NULL, NULL, &config, PRE_KERNEL_1, CONFIG_RESET_INIT_PRIORITY, &reset_ft_driver_api);
